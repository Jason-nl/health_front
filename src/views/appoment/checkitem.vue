...
<script>
import {findPage,saveOrUpdate,deleteCheckItem} from '@/api/checkitem'
export default {
  data() {
    return {
      pagination: {//分页相关模型数据
  ...
  methods: {
    ...
    //提交更新数据
    handleEdit(roleForm) {
      //因为新增和更新用的是同一个接口，所以直接调用handleAdd函数
      this.handleAdd(roleForm);
    },
    async handleDelete(row) {
      //删除后重新分页查询
      //   console.log(row);
      let res = await deleteCheckItem(row.id);
